...
class PacketTraceGenerator:
  def __init__(self, env, id, filename, initial_delay=0, finish=float('inf'), flow_id, rec_flow=False):
    self.id = id
    self.env = env
  ...
  def run(self):
    ...
    try:
      while self.env.now < self.finish:
        ...
        self.packets_sent += 1
        p = Packet(self.env.now, size, packet_id, src=self.id, flow_id=flow_id)
        if self.rec_flow:
