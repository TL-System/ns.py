...
class REDPort(SwitchPort):
  def __init__(self, env, rate, qlimit, limit_bytes, zero_downstream_buffer, debug, node_id,  max_threshold, min_threshold, max_probability, weight_factor=9):
      super().__init__(env, rate, qlimit=qlimit, limit_bytes=limit_bytes, zero_downstream_buffer=zero_downstream_buffer, debug=debug, node_id=node_id)
  ...
  def put(self, packet):
    ...
    if self.average_queue_size >= self.qlimit:
      ...
      if self.debug:
        print(f"Avg queue size ({self.average_queue_size}) exceeds threshold ({self.qlimit})")
    elif self.average_queue_size >= self.max_threshold:
      rand = random.uniform(0,1)
      if rand <= self.max_probability:
        self.packets_dropped +=1
        if self.debug:
          print(f"Avg queue size ({self.average_queue_size}) exceeds threshold ({self.qlimit})", f"Packet dropped with probability {self.max_probability}")
      else:
      ...
      if rand <= prob:
        self.packets_dropped += 1
        if self.debug:
          print(f"Avg queue size ({self.average_queue_size}) exceeds min threshold", f"Packet dropped with probability {prob}")
      else:
        self.byte_size += packet.size
        if self.zero_downstream_buffer:
          self.downstream_store.put(packet)
