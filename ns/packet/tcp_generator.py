...
class TCPPacketGenerator:
        ...
        while self.env.now < self.flow.finish_time:
            ...
            if self.next_seq + self.mss > min(
                ...
                self.congestion_control.rs.send_packet(
                    packet, self.congestion_control.C,
                    self.max_ack - self.next_seq, self.env.now)
                self.congestion_control.next_departure_time = self.env.now
                ...
                if self.debug:
                    print(
                        ...
                        "flow_id {:d} at time {:.4f}, delivered_time {:.4f}.".
                        format(packet.packet_id, packet.size, packet.flow_id,
                ...
                if self.timer is None:
                    self.timer = Timer(self.env, 0, self.timeout_callback,
                                       self.rto)
    ...
    def timeout_callback(self):
        ...
        self.congestion_control.set_before_control(self.env.now,
        ...
        self.congestion_control.rs.send_packet(resent_pkt,
        ...
        if self.debug:
            print(
                "to Resending packet {:d} with flow_id {:d} at time {:.4f} with a timeout time {:4f}."
                .format(resent_pkt.packet_id, resent_pkt.flow_id, self.env.now,
    ...
    def put(self, ack):
        ...
        if self.max_ack == 0:
            ...
            self.rto = min(
                self.rtt_estimate +
                max(4 * self.est_deviation, self.granularity), 60)
        ...
        if self.dupack == 2:
            ...
            resent_pkt.time = self.env.now
            self.congestion_control.rs.send_packet(
                resent_pkt, self.congestion_control.C,
                self.max_ack - self.next_seq, self.env.now)
            assert resent_pkt.delivered_time > 0
            # self.congestion_control.next_departure_time = self.env.now
            # if(self.congestion_control.pacing_rate > 0):
            ...
            if self.debug:
                print(
                    "dup Resending packet {:d} with flow_id {:d} at time {:.4f}."
            ...
            for i in range(self.max_ack, ack.ack, self.mss):
                if i in self.sent_packets.keys():
                    ...
                    self.congestion_control.rs.updaterate_sample(
                        self.sent_packets[i], self.congestion_control.C,
                        self.env.now)
